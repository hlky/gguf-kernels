...
- `gguf_np.quantize()` now covers the previously missing K-quants and IQ formats by delegating those encoders to a vendored GGML reference shim. Build it with `python scripts/build_ggml_ref_lib.py`. `GGUF_GGML_LIB` remains available as an override for a compatible external shared library.
...
- For `gguf_np`, dequantization remains self-contained NumPy. Quantization for `Q2_K`, `Q3_K`, `Q4_K`, `Q5_K`, `Q6_K`, `IQ2_XXS`, `IQ2_XS`, `IQ2_S`, `IQ3_XXS`, `IQ3_S`, `IQ1_S`, `IQ1_M`, `IQ4_NL`, and `IQ4_XS` now uses the vendored GGML reference shim through `gguf_kernels.ggml_ref`.

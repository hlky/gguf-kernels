...
static inline int iq2_data_index(enum ggml_type type)
{
  ...
  return type == GGML_TYPE_IQ2_XXS ? 0 : type == GGML_TYPE_IQ2_XS                         ? 1
...
{
  assert(type == GGML_TYPE_IQ2_XXS || type == GGML_TYPE_IQ2_XS || type == GGML_TYPE_IQ1_S || type == GGML_TYPE_IQ1_M || type == GGML_TYPE_IQ2_S);
  return type == GGML_TYPE_IQ2_XXS ? 256 : type == GGML_TYPE_IQ2_XS                         ? 512
...
void iq2xs_init_impl(enum ggml_type type)
{
  ...
  const int nwant = type == GGML_TYPE_IQ1_S || type == GGML_TYPE_IQ1_M ? 3 : type == GGML_TYPE_IQ2_S ? 1
                                                                                                     : 2;
  const uint16_t *kgrid = type == GGML_TYPE_IQ2_XXS ? kgrid_2bit_256 : type == GGML_TYPE_IQ2_XS                         ? kgrid_2bit_512
...
void iq2xs_free_impl(enum ggml_type type)
{
  ...
  if (iq2_data[gindex].grid)
  {
    ...
    iq2_data[gindex].grid = nullptr;
    free(iq2_data[gindex].map);
    iq2_data[gindex].map = nullptr;
    free(iq2_data[gindex].neighbours);
    iq2_data[gindex].neighbours = nullptr;
  }
}
...
void iq3xs_free_impl(int grid_size)
{
  ...
  if (iq3_data[gindex].grid)
  {
    ...
    iq3_data[gindex].grid = nullptr;
    free(iq3_data[gindex].map);
    iq3_data[gindex].map = nullptr;
    free(iq3_data[gindex].neighbours);
    iq3_data[gindex].neighbours = nullptr;
  }
}
...
size_t quantize_iq4_xs(const float *RESTRICT src, void *RESTRICT dst, int64_t nrow, int64_t n_per_row, const float *quant_weights)
{
  ...
  for (int64_t row = 0; row < nrow; ++row)
  {
    ...
    for (int ibl = 0; ibl < nblock; ++ibl)
    {
      const float *qw = quant_weights ? quant_weights + QK_K * ibl : nullptr;
...
{
  assert(k % QK_K == 0);
  quantize_iq4_xs(x, y, 1, k, nullptr);
}
...
bool ggml_validate_row_data(enum ggml_type type, const void *data, size_t nbytes)
{
  ...
  if (nbytes % libgguf_type_size(type) != 0)
  {
    fprintf(stderr, "%s: invalid size %zu for type %s (type size = %zu)\n", __func__, nbytes, libgguf_type_name(type), libgguf_type_size(type));
    return false;
  }

  const size_t nb = nbytes / libgguf_type_size(type);
